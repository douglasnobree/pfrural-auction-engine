...
import { Prisma, type BidOrigin as PrismaBidOrigin } from '@prisma/client';
...
export class BiddingService {
  ...
  async placeBid(input: PlaceBidInput): Promise<BidCommandResult> {
    ...
    const result = await this.database.transaction(async (client) => {
      const request = await this.findOrCreateRequest(client, input, amountCents, origin);
      if (request.existing) {
      ...
      try {
        ...
        if (lot.auction.mode === 'LIVE' && lot.auction.approvalMode === 'MANUAL_FIFO') {
          ...
          await appendDomainEvent(client, {
            ...
            payload: { bidRequestId: request.row.id, lotId: lot.id, externalLotId: lot.externalLotId, receivedAt: new Date().toISOString() }, writeEventLog: false,
          });
          return pending;
        }
      return this.acceptRequestLocked(client, lot, request.row.id, amountCents, origin, input, undefined);
      } catch (error) {
        if (!isDomainError(error)) throw error;
        const rejected = this.rejectedResult(request.row.id, lot, error.code);
  ...
  async approveBidRequest(bidRequestId: string, actorId: string, correlationId: string): Promise<BidCommandResult> {
    return this.database.transaction(async (client) => {
      ...
      return this.acceptRequestLocked(client, lot, request.id, request.requestedAmountCents, request.origin, { lotId: lot.id, userId: request.userId, amountCents: request.requestedAmountCents.toString(), idempotencyKey: request.id, correlationId, actorId, displayName: request.displayName ?? undefined }, actorId);
    });
  }
  ...
  async rejectBidRequest(bidRequestId: string, actorId: string, reason: string, correlationId: string): Promise<BidCommandResult> {
    ...
    return this.database.transaction(async (client) => {
      ...
      if (request.status !== 'PENDING_APPROVAL') throw new DomainError('BID_NOT_PENDING', 'Bid request is not pending approval', 409);
      const lot = await this.lockLot(client, request.lotId);
      const rejected = this.rejectedResult(request.id, lot, 'MANAGER_REJECTED');
  ...
  async listEffectiveBids(lotId: string, limit = 100): Promise<Array<Record<string, string | null>>> {
    const rows = await this.database.prisma.effectiveBid.findMany({ where: { lotId }, include: { lot: { select: { auctionId: true } }, bidIntent: { select: { displayName: true } } }, orderBy: { lotSequence: 'desc' }, take: limit });
    return rows.map((row) => ({ id: row.id, amountCents: row.amountCents.toString(), origin: row.origin, lotSequence: row.lotSequence.toString(), createdAt: row.createdAt.toISOString(), bidderAlias: participantAlias(row.lot.auctionId, row.userId, row.bidIntent.displayName) }));
  }

  private async findOrCreateRequest(client: PrismaTransaction, input: PlaceBidInput, amountCents: bigint, origin: BidOrigin): Promise<{ row: { id: string; requestedAmountCents: bigint; result: unknown }; existing: boolean }> {
    ...
    try {
      const created = await client.bidRequest.create({ data: {
        ...
        requestedAmountCents: amountCents, origin: origin as PrismaBidOrigin, actorId: input.actorId, expectedVersion: input.expectedVersion,
      } });
  ...
  private async acceptRequestLocked(client: PrismaTransaction, lot: LockedLot, bidRequestId: string, amountCents: bigint, origin: BidOrigin, input: PlaceBidInput, approvalActorId: string | undefined): Promise<BidCommandResult> {
    const nextSequence = lot.lotSequence + 1n;
    const intent = await client.bidIntent.create({ data: {
      ...
      actorId: approvalActorId ?? input.actorId, intentSequence: nextSequence, approvedAt: new Date(),
    } });
    ...
    if (priceChanged || leaderChanged) {
      const effective = await client.effectiveBid.create({ data: {
        ...
        amountCents: effectivePriceCents, lotSequence: nextSequence, causedByIntentId: intent.id,
      } });
    ...
    const result: BidCommandResult = {
      ...
      endsAt: endsAt?.toISOString() ?? null, serverTime: new Date().toISOString(),
      ...(origin === 'PROXY' ? { proxyMaxBidCents: amountCents.toString() } : {}),
      ...(effectiveBidId ? { effectiveBidId } : {}), ...(timerExtended ? { timerExtended: true } : {}),
    };
    await client.bidRequest.update({ where: { id: bidRequestId }, data: { status: 'ACCEPTED', result: result as unknown as Prisma.InputJsonValue, completedAt: new Date() } });
    await appendDomainEvent(client, {
      ...
      payload: { bidRequestId, lotId: lot.id, externalLotId: lot.externalLotId, lotSequence: nextSequence.toString(), version: newVersion.toString(), currentPriceCents: effectivePriceCents.toString(), nextBidCents: (effectivePriceCents + lot.incrementCents).toString(), currentBidderAlias: participantAlias(lot.auctionId, evaluation.leader.userId, evaluation.leader.displayName), currentBidderName: evaluation.leader.displayName ?? null, bidOrigin: evaluation.leader.origin, endsAt: endsAt?.toISOString() ?? null, timerExtended, serverTime: result.serverTime },
    });
    return result;
  }

  private pendingResult(requestId: string, lot: LockedLot): BidCommandResult {
    return { status: 'PENDING_APPROVAL', bidRequestId: requestId, lotId: lot.id, lotSequence: lot.lotSequence.toString(), version: lot.version.toString(), currentPriceCents: lot.currentPriceCents?.toString() ?? null, nextBidCents: (lot.currentPriceCents === null ? (lot.startingBidCents > 0n ? lot.startingBidCents : lot.incrementCents) : lot.currentPriceCents + lot.incrementCents).toString(), currentBidderAlias: lot.currentBidderAlias, endsAt: lot.endsAt?.toISOString() ?? null, serverTime: new Date().toISOString() };
  }

  private rejectedResult(requestId: string, lot: LockedLot, code: string): BidCommandResult {
    return { status: 'REJECTED', bidRequestId: requestId, lotId: lot.id, lotSequence: lot.lotSequence.toString(), version: lot.version.toString(), currentPriceCents: lot.currentPriceCents?.toString() ?? null, nextBidCents: (lot.currentPriceCents === null ? (lot.startingBidCents > 0n ? lot.startingBidCents : lot.incrementCents) : lot.currentPriceCents + lot.incrementCents).toString(), currentBidderAlias: lot.currentBidderAlias, endsAt: lot.endsAt?.toISOString() ?? null, serverTime: new Date().toISOString(), errorCode: code };
  }
